from flask import url_for, g, Markup, current_app
...
def import_c3():
    ...
    import_c3_css = '<link type="text/css" rel="stylesheet" href="%s"/>' \
        % url_for('static', filename=current_app.config.get(
                                            'C3_CSS_PATH', 'css/c3.min.css'))
    import_js_d3 = '<script type="text/javascript" src="%s"></script>' % \
        url_for('static', filename=current_app.config.get(
                                            'D3_JS_PATH', 'js/d3.v3.min.js'))
    import_js_c3 = '<script type="text/javascript" src="%s"></script>' % \
        url_for('static', filename=current_app.config.get(
                                            'C3_JS_PATH', 'js/c3.min.css'))
...
def register(app):
    ...
    def c3_processor():
        #######################################################################
        def pie(
                bind_to, data, title='', show_legend=True, height=None,
                width=None
                ):
            ...
            Args:
                ...
                bind_to: A string that specifics an HTML element
                    (eg: id or class) that chart will be shown in that.
                    (like: '#chart')
                ...
                show_legend: It's boolean option, If false, legends of the
                    chart will be hidden.
                height: It's an integer option, It will determine heigth of
                    chart in pixel.
                width: It's an integer option, It will determine width of chart
                    in pixel.

            Returns:
                A string contains chart js code and import code of C3 static
                files, if it did not imported yet.
            ...
            # convert parameters to strings to be acceptable in JS and C3
            if show_legend:
                show_legend = 'true'
            else:
                show_legend = 'false'
            ...
            # add import C3 elements to it if it does not imported yet
            #   and return it.
            if not (hasattr(g, 'c3_imported') and g.c3_imported):
                g.c3_imported = True
                return Markup('%s\n%s' % (import_c3(), chart))
            else:
                return Markup(chart)
        ...
        def step(
                bind_to, data, title='', area=False, x_is_category=False,
                labels=False, vertical_grid_line=False,
                horizontal_grid_line=False, show_legend=True, zoom=False,
                group_tooltip=True, height=None, width=None
                ):
            ...
            """Generates javascript code to show a 'step' chart.
                ...
                Args:
                    ...
                    data: It is dictinary that contains data of chart, some
                        ...
                        {
                            'x': ['2017-5-19', '2017-5-20', '2017-5-21'],
                            'horizontal_lines': [40],
                            # 'vertical_lines': [40],
                            'data': [
                                {'title': 'A', 'values': [26, 35, 52],
                                    'color': '#FF34FF'},
                                # {'title': 'B', 'values': [54, 25, 52]},
                            ],
                            # 'groups': [('A', 'B')]
                        }
                        vertical_lines works just if x_is_category seted
                        to False.
                    title: A string that will be shown on top of the chart.
                    area: It's a boolean option. If true, the area under the
                        curve will be colored.
                    x_is_category: It's a boolean option. If false, labels of
                        X axis will be considered as real number and sortable.
                        (they will be sorted automatically)
                    labels: It's a boolean option. If true,
                        value of record will be shown on column.
                    vertical_grid_line: It's boolean option, If true some
                        vertical rows will be drawn in chart. (grid lines)
                    horizontal_grid_line: It's boolean option, If true some
                        horizontal rows will be drawn in chart. (grid lines)
                    show_legend: It's boolean option, If false, legends of the
                        chart will be hidden.
                    zoom: It's boolean option, If true, end user can scroll on
                        chart to zoom in and zoom out.
                    group_tooltip: It's boolean option, If true, data of all
                        records in that point whill be shown to gather.
                    height: It's an integer option, it will determine heigth
                        of chart in pixel.
                    width: It's an integer option, it will determine width of
                        chart in pixel.
            ...
            # raise an exception if x_is_category set to true
                # and vertical_lines exists
            vertical_lines = str()
            if 'vertical_lines' in data.keys():
                if x_is_category:
                    raise Exception("It's meaningless to use vertical_lines"
                                    "with x_is_category.")
            ...
            if 'x' in data.keys():
                ...
                x_labels = \
                    '["2d2014226823e74c2accfcce8e0ca141", %s],' % x_labels
            ...
            if 'groups' in data.keys():
                for group in data['groups']:
                    group_string = str()
                    for item in group:
                        # raise an exception if mentioned key
                            # were not exist in data
                        if item not in data_title_list:
                            raise ValueError(
                                "%s is not exists in your data!" % item)
        ...
        def line_xy(
                bind_to, data, title='', angle=True, area=False, labels=False,
                vertical_grid_line=False, horizontal_grid_line=False,
                show_legend=True, zoom=False, show_points=True,
                group_tooltip=True, height=None, width=None
                ):
            ...
            """Generates javascript code to show a 'bar' chart.
                ...
                Args:
                    ...
                    area: It's a boolean option. If true, the area under the
                        curve will be colored.
                    angle: It's a boolean option. If false, chart type will be
                        spline.
                    labels: It's a boolean option. If true, value of record
                        will be shown on column.
                    vertical_grid_line: It's boolean option, If true some
                        vertical rows will be drawn in chart. (grid lines)
                    horizontal_grid_line: It's boolean option, If true some
                        horizontal rows will be drawn in chart. (grid lines)
                    show_legend: It's boolean option, If false, legends of
                        the chart will be hidden.
                    zoom: It's boolean option, If true, end user can scroll on
                        chart to zoom in and zoom out.
                    group_tooltip: It's boolean option, If true, data of all
                        records in that point whill be shown to gather.
                    ...
                    height: It's an integer option, It will determine heigth
                        of chart in pixel.
                    width: It's an integer option, It will determine width
                        of chart in pixel
            ...
            chart = chart_structur % (
                    ...
                    horizontal_lines, show_legend, zoom, show_points,
                    group_tooltip, height, width
                    )
        ...
        def line(
                bind_to, data, title='', angle=True, area=False,
                ...
                Args:
                    ...
                    data: It is dictinary that contains data of chart, some
                        ...
                        chart axis labels. eg:
                        {
                            ...
                            # 'vertical_lines': [40],
                            'data': [
                                {'title': 'A', 'values': [26, 5, 52]},
                                {'title': 'B', 'values': [54, 21, 40]},
                                {'title': 'C', 'values': [63, 14, 25]},
                            ],
                            # 'groups': [('B', 'C')]
                        }
                        vertical_lines works just if x_is_category seted
            ...
            # read vertical line points from data
            # raise an exception if x_is_category set to
                # true and vertical_lines exists
            vertical_lines = str()
            if 'vertical_lines' in data.keys():
                if x_is_category:
                    raise Exception("It's meaningless to use vertical_lines"
        ...
        def bar(
                bind_to, data, title='', x_is_category=False, labels=False,
                vertical_grid_line=False, horizontal_grid_line=False,
                show_legend=True, zoom=False, group_tooltip=True,
                column_width=None, height=None, width=None
                ):
            ...
            Args:
                ...
                title: A string that will be shown on top of the chart.
                column_width: It's an integer option. It will determine
                    width of chart columns in pixel.
                x_is_category: It's a boolean option. If false, labels of
                    X axis will be considered as real number and sortable.
                    (they will be sorted automatically)
                labels: It's a boolean option. If true, value of record will be
                    shown on column.
                vertical_grid_line: It's boolean option, If true some vertical
            ...
            chart = chart_structur % (
                ...
                vertical_grid_line, vertical_lines, horizontal_grid_line,
                horizontal_lines, show_legend, zoom, group_tooltip, height,
                width
                )
        ...
        def donut(
                bind_to, data, inner_title='', outer_title='',
            ...
            # pass arguments to chart structure
            chart = chart_structur % (
                        bind_to, chart_data, chart_color, outer_title,
                        inner_title, show_legend, height, width
                        )
        ...
        return dict(line=line, donut=donut, bar=bar,
                    pie=pie, step=step, line_xy=line_xy)
